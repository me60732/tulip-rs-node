...
function main() {
  ...
  const _nBase = ti.apo.info.outputs.length;
  const _optNames = ti.apo.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.apo.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.apo.indicator([close], options, _optNames.map((_, i) => i === 0));
