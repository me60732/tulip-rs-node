...
function main() {
  ...
  console.log('='.repeat(60));
  // Available: 'ema'
  const _nBase = ti.dema.info.outputs.length;
  const _optNames = ti.dema.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.dema.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.dema.indicator([close], options, _optNames.map((_, i) => i === 0));
