/**
 * Node.js example for the ROC indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic ROC calculation
...
function main() {
  ...
  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.roc.indicator([close], options);
  console.log(`Full ROC Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.roc.indicator([partialClose], options);
  console.log(`\nPartial ROC Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: ROC State - internal state for Rate of Change');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final ROC Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.roc.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} ROC values: ${output[0]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.roc.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.roc.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} ROC values (first 5): ${output[0].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.roc.indicator([expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'mom'
  const _nBase = ti.roc.info.outputs.length;
  const _optNames = ti.roc.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.roc.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.roc.indicator([close], options, _optNames.map((_, i) => i === 0));
