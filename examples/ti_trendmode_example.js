/**
 * Node.js example for the TrendMode indicator from tulip-rs-node.
 * TrendMode detects whether price is in a trending or cycling regime.
 * Optional outputs: cycle, peak. Requires ~56 bars (use close80).
 */
...
function main() {
  ...
  console.log(`Minimum data required: ${ti.trendmode.minData(options)}`);
  console.log();

  const [outputs] = ti.trendmode.indicator([close80], options);
  console.log(`Full TrendMode Value: ${outputs[0]}`);

  const n = close80.length - 5;
  const [outputs2, state2] = ti.trendmode.indicator([close80.slice(0, n)], options);
  console.log(`\nPartial TrendMode Value: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: TrendMode State - internal state for Trend Mode');
  const finalOutputs = state2.batchIndicator([close80.slice(n)]);
  console.log(`Final TrendMode Value: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.trendmode.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} TrendMode Value: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.trendmode.indicator(inp, options);
  ...
  console.log('='.repeat(60));
  const simdOptions = [[0.0], [0.05], [0.07], [0.10]];
  simdOptions.forEach((opt, i) => console.log(`Option set ${i + 1}: ${JSON.stringify(opt)}`));
  console.log();
  try {
    const [simdOptOutputs] = ti.trendmode.simdByOptions([close80], simdOptions);
    simdOptOutputs.forEach((output, i) =>
      console.log(`Option set ${i + 1} TrendMode (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.trendmode.indicator([close80], opt);
  ...
  console.log('='.repeat(60));
  const _nBase = ti.trendmode.info.outputs.length;
  const _optNames = ti.trendmode.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  const [_allOut] = ti.trendmode.indicator([close80], options, _optNames.map(() => true));
  console.log('All optional outputs enabled:');
  _optNames.forEach((n, i) => {
    console.log(`  ${n}: ${_allOut[_nBase + i]}`);
  });

  const [_firstOut] = ti.trendmode.indicator([close80], options, _optNames.map((_, i) => i === 0));
