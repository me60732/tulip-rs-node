/**
 * Node.js example for the CMO indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic CMO calculation
...
function main() {
  ...
  const minData = ti.cmo.minData(options);
  console.log(`Minimum data required: ${minData}`);

  const minDataAccuracy = ti.cmo.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.cmo.indicator([close], options);
  console.log(`Full CMO Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.cmo.indicator([partialClose], options);
  console.log(`\nPartial CMO Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: CMO State - internal state for Chande Momentum Oscillator');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final CMO Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.cmo.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} CMO values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.cmo.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.cmo.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} CMO values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.cmo.indicator([expandedClose], opt);
