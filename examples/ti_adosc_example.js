...
function main() {
  ...
  console.log(`Minimum data required: ${ti.adosc.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.adosc.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.adosc.indicator([high, low, close, volume], options);
  console.log(`Full ADOSC Line: ${outputs[0]}`);

  const n = high.length - 5;
  const [outputs2, state2] = ti.adosc.indicator(
    [high.slice(0, n), low.slice(0, n), close.slice(0, n), volume.slice(0, n)],
    options
  );
  console.log(`\nPartial ADOSC Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: ADOSC State - internal state for Accumulation/Distribution Oscillator');
  const finalOutputs = state2.batchIndicator([high.slice(n), low.slice(n), close.slice(n), volume.slice(n)]);
  console.log(`Final ADOSC Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.adosc.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} ADOSC values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.adosc.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.adosc.simdByOptions([expandedHigh, expandedLow, expandedClose, expandedVolume], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} ADOSC values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.adosc.indicator([expandedHigh, expandedLow, expandedClose, expandedVolume], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'short_ema', 'long_ema', 'ad'
  const _nBase = ti.adosc.info.outputs.length;
  const _optNames = ti.adosc.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.adosc.indicator([high, low, close, volume], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.adosc.indicator([high, low, close, volume], options, _optNames.map((_, i) => i === 0));
