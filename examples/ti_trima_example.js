/**
 * Node.js example for the TRIMA indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic TRIMA calculation
...
function main() {
  ...
  const minData = ti.trima.minData(options);
  console.log(`Minimum data required: ${minData}`);
  const minDataAccuracy = ti.trima.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.trima.indicator([close], options);
  console.log(`Full TRIMA Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.trima.indicator([partialClose], options);
  console.log(`\nPartial TRIMA Line: ${outputs2[0]}`);

  console.log("\nDemonstrating state continuation...");
  console.log(
    "State info: TRIMA State - internal state for Triangular Moving Average",
  );

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final TRIMA Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.trima.simdByAssets(simdInputs, options);
    console.log("SIMD Results:");
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} TRIMA values: ${output[0]}`);
    });
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.trima.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.trima.simdByOptions(
      [expandedClose],
      simdOptions,
    );
    console.log("SIMD Results:");
    simdOptOutputs.forEach((output, i) => {
      console.log(
        `Option set ${i + 1} TRIMA values (first 5): ${output[0].slice(0, 5)}`,
      );
    });
    console.log("\nVerification - calculating each option set individually:");
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.trima.indicator([expandedClose], opt);
      console.log(
