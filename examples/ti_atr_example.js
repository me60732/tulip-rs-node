...
function main() {
  ...
  console.log('='.repeat(60));
  // Available: 'tr'
  const _nBase = ti.atr.info.outputs.length;
  const _optNames = ti.atr.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.atr.indicator([high, low, close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.atr.indicator([high, low, close], options, _optNames.map((_, i) => i === 0));
