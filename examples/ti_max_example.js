/**
 * Node.js example for the MAX indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic MAX calculation
...
function main() {
  ...
  const minData = ti.max.minData(options);
  console.log(`Minimum data required: ${minData}`);
  const minDataAccuracy = ti.max.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.max.indicator([close], options);
  console.log(`Full MAX Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.max.indicator([partialClose], options);
  console.log(`\nPartial MAX Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: MAX State - internal state for Maximum In Period');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final MAX Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.max.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} MAX values: ${output[0]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.max.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.max.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} MAX values (first 5): ${output[0].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.max.indicator([expandedClose], opt);
