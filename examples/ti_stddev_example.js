/**
 * Node.js example for the STDDEV indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic STDDEV calculation
...
function main() {
  ...
  const minData = ti.stddev.minData(options);
  console.log(`Minimum data required: ${minData}`);
  const minDataAccuracy = ti.stddev.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.stddev.indicator([close], options);
  console.log(`Full Std Dev Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.stddev.indicator([partialClose], options);
  console.log(`\nPartial Std Dev Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: STDDEV State - internal state for Standard Deviation');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final Std Dev Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.stddev.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} STDDEV values: ${output[0]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.stddev.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.stddev.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} STDDEV values (first 5): ${output[0].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.stddev.indicator([expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'sma'
  const _nBase = ti.stddev.info.outputs.length;
  const _optNames = ti.stddev.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.stddev.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.stddev.indicator([close], options, _optNames.map((_, i) => i === 0));
