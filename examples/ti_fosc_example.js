/**
 * Node.js example for the FOSC indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic FOSC calculation
...
function main() {
  ...
  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.fosc.indicator([close], options);
  console.log(`Full FOSC Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.fosc.indicator([partialClose], options);
  console.log(`\nPartial FOSC Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: FOSC State - internal state for Forecast Oscillator');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final FOSC Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.fosc.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} FOSC values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.fosc.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.fosc.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} FOSC values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.fosc.indicator([expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'tsf', 'linreg', 'linregslope', 'linregintercept'
  const _nBase = ti.fosc.info.outputs.length;
  const _optNames = ti.fosc.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.fosc.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.fosc.indicator([close], options, _optNames.map((_, i) => i === 0));
  console.log(`\nOnly '${_optNames[0]}' enabled:`);
  console.log(`  ${_optNames[0]}: ${_firstOut[_nBase]}`);
  console.log(`  linreg: [] (not requested)`);
}
