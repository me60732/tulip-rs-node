...
function main() {
  ...
  console.log(`Full TSF Line: ${outputs[0]}`);

  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.tsf.indicator([partialClose], options);
  console.log(`\nPartial TSF Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: TSF State - internal state for Time Series Forecast');
  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final TSF Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.tsf.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} TSF values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.tsf.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.tsf.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} TSF values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.tsf.indicator([expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'linreg', 'linregslope', 'linregintercept'
  const _nBase = ti.tsf.info.outputs.length;
  const _optNames = ti.tsf.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.tsf.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.tsf.indicator([close], options, _optNames.map((_, i) => i === 0));
  console.log(`\nOnly '${_optNames[0]}' enabled:`);
  console.log(`  ${_optNames[0]}: ${_firstOut[_nBase]}`);
  console.log(`  linregslope: [] (not requested)`);
}
