...
function main() {
  ...
  const [outputs] = ti.wad.indicator([high, low, close], options);
  console.log(`Full WAD Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.wad.indicator(
    [partialHigh, partialLow, partialClose],
    options,
  );
  console.log(`\nPartial WAD Line: ${outputs2[0]}`);
  console.log("\nDemonstrating state continuation...");
  console.log(
    "State info: WAD State - internal state for Williams Accumulation/Distribution",
  );
  ...
  console.log(`Final WAD Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.wad.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} WAD values: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.wad.indicator(inp, options);
