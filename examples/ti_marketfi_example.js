...
function main() {
  ...
  const [outputs] = ti.marketfi.indicator([high, low, volume], options);
  console.log(`Full Market Facilitation Index Line: ${outputs[0]}`);

  const n = high.length - 5;
  const [outputs2, state2] = ti.marketfi.indicator(
    [high.slice(0, n), low.slice(0, n), volume.slice(0, n)],
    options,
  );
  console.log(`\nPartial Market Facilitation Index Line: ${outputs2[0]}`);
  ...
  console.log(`Final Market Facilitation Index Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.marketfi.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(
        `Asset ${i + 1} Market Facilitation Index values: ${output[0]}`,
      ),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.marketfi.indicator(inp, options);
