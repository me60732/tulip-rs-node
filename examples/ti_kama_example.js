/**
 * Node.js example for the KAMA indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic KAMA calculation
...
function main() {
  ...
  const minData = ti.kama.minData(options);
  console.log(`Minimum data required: ${minData}`);

  const minDataAccuracy = ti.kama.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.kama.indicator([close], options);
  console.log(`Full KAMA Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.kama.indicator([partialClose], options);
  console.log(`\nPartial KAMA Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: KAMA State - internal state for Kaufman Adaptive Moving Average');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final KAMA Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.kama.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} KAMA values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.kama.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.kama.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} KAMA values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.kama.indicator([expandedClose], opt);
