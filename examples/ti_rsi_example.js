/**
 * Node.js example for the RSI indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic RSI calculation
...
function main() {
  ...
  const minData = ti.rsi.minData(options);
  console.log(`Minimum data required: ${minData}`);

  const minDataAccuracy = ti.rsi.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.rsi.indicator([close], options);
  console.log(`Full RSI Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.rsi.indicator([partialClose], options);
  console.log(`\nPartial RSI Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: RSI State - internal state for Relative Strength Index');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final RSI Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.rsi.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} RSI values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.rsi.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.rsi.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} RSI values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.rsi.indicator([expandedClose], opt);
