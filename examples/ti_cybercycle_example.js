/**
 * Node.js example for the CyberCycle indicator from tulip-rs-node.
 * The Cyber Cycle isolates the dominant market cycle using a high-pass filter.
 * Optional output: trigger.
 */
...
function main() {
  ...
  console.log(`Minimum data required: ${ti.cybercycle.minData(options)}`);
  console.log();

  const [outputs] = ti.cybercycle.indicator([close], options);
  console.log(`Full CyberCycle Value: ${outputs[0]}`);

  const n = close.length - 5;
  const [outputs2, state2] = ti.cybercycle.indicator([close.slice(0, n)], options);
  console.log(`\nPartial CyberCycle Value: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: CyberCycle State - internal state for Cyber Cycle');
  const finalOutputs = state2.batchIndicator([close.slice(n)]);
  console.log(`Final CyberCycle Value: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.cybercycle.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} CyberCycle Value: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.cybercycle.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.cybercycle.simdByOptions([close], simdOptions);
    simdOptOutputs.forEach((output, i) =>
      console.log(`Option set ${i + 1} CyberCycle (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.cybercycle.indicator([close], opt);
  ...
  console.log('='.repeat(60));
  const _nBase = ti.cybercycle.info.outputs.length;
  const _optNames = ti.cybercycle.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  const [_allOut] = ti.cybercycle.indicator([close], options, _optNames.map(() => true));
  console.log('All optional outputs enabled:');
  _optNames.forEach((n, i) => {
    console.log(`  ${n}: ${_allOut[_nBase + i]}`);
  });

  const [_firstOut] = ti.cybercycle.indicator([close], options, _optNames.map((_, i) => i === 0));
