/**
 * Node.js example for the TEMA indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic TEMA calculation
...
function main() {
  ...
  const minData = ti.tema.minData(options);
  console.log(`Minimum data required: ${minData}`);
  const minDataAccuracy = ti.tema.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.tema.indicator([close], options);
  console.log(`Full TEMA Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  // tema needs minData=13, so cap the split so partialClose always has enough data
  const splitPoint = Math.min(5, close.length - minData);
  const partialClose = close.slice(0, -splitPoint);
  const [outputs2, state2] = ti.tema.indicator([partialClose], options);
  ...
  console.log(`Final TEMA Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.tema.simdByAssets(simdInputs, options);
    console.log("SIMD Results:");
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} TEMA values: ${output[0]}`);
    });
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.tema.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.tema.simdByOptions(
      [expandedClose],
      simdOptions,
    );
    console.log("SIMD Results:");
    simdOptOutputs.forEach((output, i) => {
      console.log(
        `Option set ${i + 1} TEMA values (first 5): ${output[0].slice(0, 5)}`,
      );
    });
    console.log("\nVerification - calculating each option set individually:");
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.tema.indicator([expandedClose], opt);
      console.log(
  ...
  console.log('='.repeat(60));
  // Available: 'dema', 'ema'
  const _nBase = ti.tema.info.outputs.length;
  const _optNames = ti.tema.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.tema.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.tema.indicator([close], options, _optNames.map((_, i) => i === 0));
  console.log(`\nOnly '${_optNames[0]}' enabled:`);
  console.log(`  ${_optNames[0]}: ${_firstOut[_nBase]}`);
  console.log(`  ema: [] (not requested)`);
}
