...
function main() {
  ...
  const [outputs] = ti.tr.indicator([high, low, close], options);
  console.log(`Full True Range Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.tr.indicator(
    [partialHigh, partialLow, partialClose],
    options,
  );
  console.log(`\nPartial True Range Line: ${outputs2[0]}`);
  console.log("\nDemonstrating state continuation...");
  console.log("State info: TR State - internal state for True Range");
  ...
  console.log(`Final True Range Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.tr.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} True Range values: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.tr.indicator(inp, options);
