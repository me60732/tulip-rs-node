...
function main() {
  ...
  const high = new Float64Array(
    Array.from({ length: 3 }).flatMap(() => Array.from(highBase)),
  );
  const low = new Float64Array(
    Array.from({ length: 3 }).flatMap(() => Array.from(lowBase)),
  );
