...
function main() {
  ...
  const [outputs] = ti.avgprice.indicator([open, high, low, close], options);
  console.log(`Full Average Price Line: ${outputs[0]}`);
  ...
  const [outputs2, state2] = ti.avgprice.indicator(
    [partialOpen, partialHigh, partialLow, partialClose],
    options,
  );
  console.log(`\nPartial Average Price Line: ${outputs2[0]}`);
  console.log("\nDemonstrating state continuation...");
  console.log("State info: AVGPRICE State - internal state for Average Price");
  ...
  console.log(`Final Average Price Line: ${finalOutputs[0]}`);
  console.log(
  ...
  const simdInputs = [
    [open.slice(), high.slice(), low.slice(), close.slice()],
    [
      open.map((v) => v * 1.2),
      high.map((v) => v * 1.2),
      low.map((v) => v * 1.2),
  ...
  try {
    const [simdOutputs] = ti.avgprice.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} Average Price values: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.avgprice.indicator(inp, options);
