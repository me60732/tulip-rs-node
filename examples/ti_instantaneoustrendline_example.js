/**
 * Node.js example for the InstantaneousTrendline indicator from tulip-rs-node.
 * The Instantaneous Trendline separates cycle from trend using the Hilbert Transform.
 * No options. Optional outputs: trigger, dc_period, alpha.
 */
...
function main() {
  ...
  const info = ti.instantaneoustrendline.info;
  ...
  console.log(`Minimum data required: ${ti.instantaneoustrendline.minData(options)}`);
  console.log();

  const [outputs] = ti.instantaneoustrendline.indicator([close], options);
  console.log(`Full InstantaneousTrendline Value: ${outputs[0]}`);

  const n = close.length - 5;
  const [outputs2, state2] = ti.instantaneoustrendline.indicator([close.slice(0, n)], options);
  console.log(`\nPartial InstantaneousTrendline Value: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: InstantaneousTrendline State - internal state for Instantaneous Trendline');
  const finalOutputs = state2.batchIndicator([close.slice(n)]);
  console.log(`Final InstantaneousTrendline Value: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.instantaneoustrendline.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} ITrendline Value: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.instantaneoustrendline.indicator(inp, options);
  ...
  console.log('='.repeat(60));
  const _nBase = ti.instantaneoustrendline.info.outputs.length;
  const _optNames = ti.instantaneoustrendline.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  const [_allOut] = ti.instantaneoustrendline.indicator([close], options, _optNames.map(() => true));
  console.log('All optional outputs enabled:');
  _optNames.forEach((n, i) => {
    console.log(`  ${n}: ${_allOut[_nBase + i]}`);
  });

  const [_firstOut] = ti.instantaneoustrendline.indicator([close], options, _optNames.map((_, i) => i === 0));
