...
function main() {
  ...
  console.log(`Minimum data required: ${ti.volatility.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.volatility.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.volatility.indicator([close], options);
  console.log(`Full Volatility Line: ${outputs[0]}`);

  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.volatility.indicator([partialClose], options);
  console.log(`\nPartial Volatility Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: VOLATILITY State - internal state for Annualized Historical Volatility');
  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final Volatility Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.volatility.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} Volatility values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.volatility.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.volatility.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} Volatility values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.volatility.indicator([expandedClose], opt);
