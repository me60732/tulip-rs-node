...
function main() {
  ...
  console.log(`Minimum data required: ${ti.natr.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.natr.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.natr.indicator([high, low, close], options);
  console.log(`Full NATR Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.natr.indicator([partialHigh, partialLow, partialClose], options);
  console.log(`\nPartial NATR Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: NATR State - internal state for Normalized Average True Range');
  ...
  console.log(`Final NATR Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.natr.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} NATR values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.natr.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.natr.simdByOptions([expandedHigh, expandedLow, expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} NATR values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.natr.indicator([expandedHigh, expandedLow, expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'atr', 'tr'
  const _nBase = ti.natr.info.outputs.length;
  const _optNames = ti.natr.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.natr.indicator([high, low, close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.natr.indicator([high, low, close], options, _optNames.map((_, i) => i === 0));
