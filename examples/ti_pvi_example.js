/**
 * Node.js example for the PVI indicator from tulip-rs-node.
 * Positive Volume Index uses Close, Volume inputs with no options.
 */
...
function main() {
  ...
  const [outputs] = ti.pvi.indicator([close, volume], []);
  console.log(`Full PVI Line: ${outputs[0]}`);

  const n = close.length - 5;
  const [outputs2, state2] = ti.pvi.indicator(
    [close.slice(0, n), volume.slice(0, n)],
    [],
  );
  console.log(`\nPartial PVI Line: ${outputs2[0]}`);
  console.log("\nDemonstrating state continuation...");
  console.log(
    "State info: PVI State - internal state for Positive Volume Index",
  );
  const finalOutputs = state2.batchIndicator([close.slice(n), volume.slice(n)]);
  console.log(`Final PVI Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.pvi.simdByAssets(simdInputs, []);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} PVI values: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.pvi.indicator(inp, []);
