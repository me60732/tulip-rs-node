/**
 * Node.js example for the MIN indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic MIN calculation
...
function main() {
  ...
  const minData = ti.min.minData(options);
  console.log(`Minimum data required: ${minData}`);
  const minDataAccuracy = ti.min.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.min.indicator([close], options);
  console.log(`Full MIN Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.min.indicator([partialClose], options);
  console.log(`\nPartial MIN Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: MIN State - internal state for Minimum In Period');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final MIN Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.min.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} MIN values: ${output[0]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.min.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.min.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} MIN values (first 5): ${output[0].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.min.indicator([expandedClose], opt);
