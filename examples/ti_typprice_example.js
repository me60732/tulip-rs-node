...
function main() {
  ...
  const [outputs] = ti.typprice.indicator([high, low, close], options);
  console.log(`Full Typical Price Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.typprice.indicator(
    [partialHigh, partialLow, partialClose],
    options,
  );
  console.log(`\nPartial Typical Price Line: ${outputs2[0]}`);
  console.log("\nDemonstrating state continuation...");
  console.log("State info: TYPPRICE State - internal state for Typical Price");
  ...
  console.log(`Final Typical Price Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.typprice.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} Typical Price values: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.typprice.indicator(inp, options);
