/**
 * Node.js example for the DPO indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic DPO calculation
...
function main() {
  ...
  const minData = ti.dpo.minData(options);
  console.log(`Minimum data required: ${minData}`);

  const minDataAccuracy = ti.dpo.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.dpo.indicator([close], options);
  console.log(`Full DPO Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.dpo.indicator([partialClose], options);
  console.log(`\nPartial DPO Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: DPO State - internal state for Detrended Price Oscillator');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final DPO Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.dpo.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} DPO values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.dpo.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.dpo.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} DPO values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.dpo.indicator([expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'sma'
  const _nBase = ti.dpo.info.outputs.length;
  const _optNames = ti.dpo.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.dpo.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.dpo.indicator([close], options, _optNames.map((_, i) => i === 0));
