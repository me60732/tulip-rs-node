/**
 * Node.js example for the HilbertTransform indicator from tulip-rs-node.
 * The Hilbert Transform decomposes price into in-phase and quadrature components.
 * Optional outputs: roofing, highpass.
 */
...
function main() {
  ...
  const info = ti.hilberttransform.info;
  ...
  console.log(`Minimum data required: ${ti.hilberttransform.minData(options)}`);
  console.log();

  const [outputs] = ti.hilberttransform.indicator([close], options);
  console.log(`Full HilbertTransform Value: ${outputs[0]}`);

  const n = close.length - 5;
  const [outputs2, state2] = ti.hilberttransform.indicator(
    [close.slice(0, n)],
    options,
  );
  console.log(`\nPartial HilbertTransform Value: ${outputs2[0]}`);

  console.log("\nDemonstrating state continuation...");
  console.log(
    "State info: HilbertTransform State - internal state for Hilbert Transform",
  );
  const finalOutputs = state2.batchIndicator([close.slice(n)]);
  console.log(`Final HilbertTransform Value: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.hilberttransform.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} HilbertTransform Value: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.hilberttransform.indicator(inp, options);
  ...
  console.log("=".repeat(60));
  // Option sets chosen so every minData <= close.length (40): formula is hp_period + 8
  const simdOptions = [
    [5, 10],
    [10, 20],
    [15, 30],
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.hilberttransform.simdByOptions(
      [close],
      simdOptions,
    );
    simdOptOutputs.forEach((output, i) =>
      console.log(
        `Option set ${i + 1} HilbertTransform (first 5): ${output[0].slice(0, 5)}`,
      ),
    );
    console.log("\nVerification - calculating each option set individually:");
    simdOptions.forEach((opt, i) => {
      const [o] = ti.hilberttransform.indicator([close], opt);
      console.log(
  ...
  console.log("=".repeat(60));
  const _nBase = ti.hilberttransform.info.outputs.length;
  const _optNames = ti.hilberttransform.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(", ")}`);
  console.log();

  const [_allOut] = ti.hilberttransform.indicator(
    [close],
    options,
    _optNames.map(() => true),
  );
  ...
  const [_firstOut] = ti.hilberttransform.indicator(
    [close],
    options,
