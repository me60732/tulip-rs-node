...
function main() {
  ...
  console.log(`Minimum data required: ${ti.cci.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.cci.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.cci.indicator([high, low, close], options);
  console.log(`Full CCI Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.cci.indicator([partialHigh, partialLow, partialClose], options);
  console.log(`\nPartial CCI Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: CCI State - internal state for Commodity Channel Index');
  ...
  console.log(`Final CCI Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.cci.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} CCI values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.cci.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.cci.simdByOptions([expandedHigh, expandedLow, expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} CCI values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.cci.indicator([expandedHigh, expandedLow, expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'sma', 'md', 'typprice'
  const _nBase = ti.cci.info.outputs.length;
  const _optNames = ti.cci.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.cci.indicator([high, low, close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.cci.indicator([high, low, close], options, _optNames.map((_, i) => i === 0));
  console.log(`\nOnly '${_optNames[0]}' enabled:`);
  console.log(`  ${_optNames[0]}: ${_firstOut[_nBase]}`);
  console.log(`  md: [] (not requested)`);
}
