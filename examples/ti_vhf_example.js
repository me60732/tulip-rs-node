...
function main() {
  ...
  console.log(`Minimum data required: ${ti.vhf.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.vhf.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.vhf.indicator([close], options);
  console.log(`Full VHF Line: ${outputs[0]}`);

  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.vhf.indicator([partialClose], options);
  console.log(`\nPartial VHF Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: VHF State - internal state for Vertical Horizontal Filter');
  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final VHF Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.vhf.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} VHF values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.vhf.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.vhf.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} VHF values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.vhf.indicator([expandedClose], opt);
