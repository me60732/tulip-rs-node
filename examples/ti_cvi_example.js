/**
 * Node.js example for the CVI indicator from tulip-rs-node.
 * CVI (Chaikin Volatility) returns one output: CVI Line.
 */
...
function main() {
  ...
  console.log(`Minimum data required: ${ti.cvi.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.cvi.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.cvi.indicator([high, low], options);
  console.log('Full dataset calculation:');
  console.log(`Full CVI Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow  = low.slice(0, -5);
  const [outputs2, state2] = ti.cvi.indicator([partialHigh, partialLow], options);
  console.log(`\nPartial calculation (first ${partialHigh.length} elements):`);
  console.log(`Partial CVI Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: CVI State - internal state for Chaikin Volatility');
  ...
  console.log(`Final CVI Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.cvi.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} CVI Line: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.cvi.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.cvi.simdByOptions([expandedHigh, expandedLow], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} CVI Line (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.cvi.indicator([expandedHigh, expandedLow], opt);
