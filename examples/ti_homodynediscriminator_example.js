/**
 * Node.js example for the HomodyneDiscriminator indicator from tulip-rs-node.
 * The Homodyne Discriminator measures the dominant cycle period using phase accumulation.
 * No options, no optional outputs.
 */
...
function main() {
  ...
  const info = ti.homodynediscriminator.info;
  ...
  console.log(`Minimum data required: ${ti.homodynediscriminator.minData(options)}`);
  console.log();

  const [outputs] = ti.homodynediscriminator.indicator([close], options);
  console.log(`Full HomodyneDiscriminator DC Period: ${outputs[0]}`);

  const n = close.length - 5;
  const [outputs2, state2] = ti.homodynediscriminator.indicator([close.slice(0, n)], options);
  console.log(`\nPartial HomodyneDiscriminator DC Period: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: HomodyneDiscriminator State - internal state for Homodyne Discriminator');
  const finalOutputs = state2.batchIndicator([close.slice(n)]);
  console.log(`Final HomodyneDiscriminator DC Period: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.homodynediscriminator.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} DC Period: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.homodynediscriminator.indicator(inp, options);
      console.log(`Asset ${i + 1} individual DC Period: ${o[0]}`);
    });
