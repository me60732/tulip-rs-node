...
function main() {
  ...
  console.log(`Minimum data for accuracy (6 decimals): ${ti.trix.minDataAccuracy(options, 6)}`);
  console.log();
  ...
  try {
    ...
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} TRIX values: ${output[0]}`));
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.trix.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} TRIX values (first 5): ${output[0].slice(0, 5)}`));
  ...
  const [_allOut] = ti.trix.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.trix.indicator([close], options, _optNames.map((_, i) => i === 0));
