...
function main() {
  ...
  console.log(`Minimum data for accuracy (6 decimals): ${ti.vidya.minDataAccuracy(options, 6)}`);
  console.log();
  ...
  console.log('State info: VIDYA State - internal state for Variable Index Dynamic Average');
  ...
  try {
    ...
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} VIDYA Line: ${output[0]}`));
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.vidya.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} VIDYA Line (first 5): ${output[0].slice(0, 5)}`));
  ...
  const [_allOut] = ti.vidya.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.vidya.indicator([close], options, _optNames.map((_, i) => i === 0));
