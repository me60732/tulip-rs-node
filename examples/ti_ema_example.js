'use strict';
/**
 * Node.js example for the EMA indicator from tulip-rs-node.
 *
...
function main() {
  ...
  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.ema.indicator([close], options);
  console.log(`Full EMA Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.ema.indicator([partialClose], options);
  console.log(`\nPartial EMA Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: EMA State - internal state for Exponential Moving Average');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final EMA Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.ema.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} EMA values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.ema.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.ema.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} EMA values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.ema.indicator([expandedClose], opt);
