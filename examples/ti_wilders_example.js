...
function main() {
  ...
  console.log(`Minimum data required: ${ti.wilders.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.wilders.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.wilders.indicator([close], options);
  console.log(`Full Wilders Line: ${outputs[0]}`);

  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.wilders.indicator([partialClose], options);
  console.log(`\nPartial Wilders Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: WILDERS State - internal state for Wilders Smoothing');
  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final Wilders Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.wilders.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} Wilders values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.wilders.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.wilders.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} Wilders values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.wilders.indicator([expandedClose], opt);
