/**
 * Node.js example for the LINREG indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic LINREG calculation
...
function main() {
  ...
  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.linreg.indicator([close], options);
  console.log(`Full Linear Regression Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.linreg.indicator([partialClose], options);
  console.log(`\nPartial Linear Regression Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: LINREG State - internal state for Linear Regression');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final Linear Regression Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.linreg.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} LINREG values: ${output[0]}`);
    });

    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.linreg.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.linreg.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} LINREG values (first 5): ${output[0].slice(0, 5)}`);
    });

    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.linreg.indicator([expandedClose], opt);
