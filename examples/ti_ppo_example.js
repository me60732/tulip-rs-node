...
function main() {
  ...
  console.log(`Minimum data required: ${ti.ppo.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.ppo.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.ppo.indicator([close], options);
  console.log('Full dataset calculation:');
  console.log(`Full PPO Line: ${outputs[0]}`);

  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.ppo.indicator([partialClose], options);
  console.log(`\nPartial calculation (first ${partialClose.length} elements):`);
  console.log(`Partial PPO Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: PPO State - internal state for Percentage Price Oscillator');
  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log('Continued calculation:');
  console.log(`Final PPO Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.ppo.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} PPO Line: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.ppo.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.ppo.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} PPO Line (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.ppo.indicator([expandedClose], opt);
  ...
  const _nBase = ti.ppo.info.outputs.length;
  const _optNames = ti.ppo.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.ppo.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.ppo.indicator([close], options, _optNames.map((_, i) => i === 0));
