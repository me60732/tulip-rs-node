...
function main() {
  ...
  console.log(`Minimum data required: ${ti.dx.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.dx.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.dx.indicator([high, low, close], options);
  console.log(`Full DX Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.dx.indicator([partialHigh, partialLow, partialClose], options);
  console.log(`\nPartial DX Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: DX State - internal state for Directional Movement Index');
  ...
  console.log(`Final DX Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.dx.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} DX values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.dx.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.dx.simdByOptions([expandedHigh, expandedLow, expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} DX values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.dx.indicator([expandedHigh, expandedLow, expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'atr', 'tr'
  const _nBase = ti.dx.info.outputs.length;
  const _optNames = ti.dx.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.dx.indicator([high, low, close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.dx.indicator([high, low, close], options, _optNames.map((_, i) => i === 0));
