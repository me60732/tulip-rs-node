/**
 * Node.js example for the ROCR indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic ROCR calculation
...
function main() {
  ...
  const minData = ti.rocr.minData(options);
  console.log(`Minimum data required: ${minData}`);
  const minDataAccuracy = ti.rocr.minDataAccuracy(options, 6);
  console.log(`Minimum data for accuracy (6 decimals): ${minDataAccuracy}`);
  console.log();

  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.rocr.indicator([close], options);
  console.log(`Full ROCR Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.rocr.indicator([partialClose], options);
  console.log(`\nPartial ROCR Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: ROCR State - internal state for Rate of Change Ratio');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final ROCR Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.rocr.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} ROCR values: ${output[0]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.rocr.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.rocr.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} ROCR values (first 5): ${output[0].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.rocr.indicator([expandedClose], opt);
