'use strict';
/**
 * Node.js example for the MD indicator from tulip-rs-node.
 *
 * This example demonstrates:
 * 1. Basic MD calculation
...
function main() {
  ...
  /////////////////////////////////////////////////// Full Calculation
  const [outputs] = ti.md.indicator([close], options);
  console.log(`Full MD Line: ${outputs[0]}`);

  /////////////////////////////////////////////////// Partial Calculation + State
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.md.indicator([partialClose], options);
  console.log(`\nPartial MD Line: ${outputs2[0]}`);

  console.log('\nDemonstrating state continuation...');
  console.log('State info: MD State - internal state for Mean Deviation');

  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final MD Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.md.simdByAssets(simdInputs, options);
    console.log('SIMD Results:');
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} MD values: ${output[0]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((assetInputs, i) => {
      const [indOutput] = ti.md.indicator(assetInputs, options);
  ...
  try {
    const [simdOptOutputs] = ti.md.simdByOptions([expandedClose], simdOptions);
    console.log('SIMD Results:');
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} MD values (first 5): ${output[0].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [indOutput] = ti.md.indicator([expandedClose], opt);
