/**
 * Node.js example for the Vortex indicator from tulip-rs-node.
 * Uses High, Low, Close inputs with a period option.
 * Outputs: vi_up, vi_down. Optional output: tr.
 */
...
function main() {
  ...
  console.log(`Minimum data required: ${ti.vortex.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.vortex.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.vortex.indicator([high, low, close], options);
  console.log(`Full Vortex VI+ Line: ${outputs[0]}`);
  console.log(`Full Vortex VI- Line: ${outputs[1]}`);

  const n = high.length - 5;
  const [outputs2, state2] = ti.vortex.indicator(
    [high.slice(0, n), low.slice(0, n), close.slice(0, n)],
    options
  );
  console.log(`\nPartial VI+: ${outputs2[0]}`);
  console.log(`Partial VI-: ${outputs2[1]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: Vortex State - internal state for Vortex Indicator');
  const finalOutputs = state2.batchIndicator([high.slice(n), low.slice(n), close.slice(n)]);
  console.log(`Final VI+: ${finalOutputs[0]}`);
  console.log(`Final VI-: ${finalOutputs[1]}`);
  ...
  try {
    const [simdOutputs] = ti.vortex.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => {
      console.log(`Asset ${i + 1} VI+: ${output[0]}`);
      console.log(`Asset ${i + 1} VI-: ${output[1]}`);
    });
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.vortex.indicator(inp, options);
      console.log(`Asset ${i + 1} VI+ individual: ${o[0]}`);
    });
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.vortex.simdByOptions([expandedHigh, expandedLow, expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => {
      console.log(`Option set ${i + 1} VI+ values (first 5): ${output[0].slice(0, 5)}`);
      console.log(`Option set ${i + 1} VI- values (first 5): ${output[1].slice(0, 5)}`);
    });
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.vortex.indicator([expandedHigh, expandedLow, expandedClose], opt);
      console.log(`Option set ${i + 1} VI+ individual (first 5): ${o[0].slice(0, 5)}`);
    });
  ...
  console.log('='.repeat(60));
  const _nBase = ti.vortex.info.outputs.length;
  const _optNames = ti.vortex.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.vortex.indicator([high, low, close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.vortex.indicator([high, low, close], options, _optNames.map((_, i) => i === 0));
