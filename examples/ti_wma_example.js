...
function main() {
  ...
  console.log(`Minimum data required: ${ti.wma.minData(options)}`);
  console.log(`Minimum data for accuracy (6 decimals): ${ti.wma.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.wma.indicator([close], options);
  console.log(`Full WMA Line: ${outputs[0]}`);

  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.wma.indicator([partialClose], options);
  console.log(`\nPartial WMA Line: ${outputs2[0]}`);
  console.log('\nDemonstrating state continuation...');
  console.log('State info: WMA State - internal state for Weighted Moving Average');
  const newClose = close.slice(-5);
  const finalOutputs = state2.batchIndicator([newClose]);
  console.log(`Final WMA Line: ${finalOutputs[0]}`);
  ...
  try {
    const [simdOutputs] = ti.wma.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} WMA values: ${output[0]}`));
    console.log('\nVerification - calculating each asset individually:');
    simdInputs.forEach((inp, i) => {
      const [o] = ti.wma.indicator(inp, options);
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.wma.simdByOptions([expandedClose], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} WMA values (first 5): ${output[0].slice(0, 5)}`));
    console.log('\nVerification - calculating each option set individually:');
    simdOptions.forEach((opt, i) => {
      const [o] = ti.wma.indicator([expandedClose], opt);
  ...
  console.log('='.repeat(60));
  // Available: 'sma'
  const _nBase = ti.wma.info.outputs.length;
  const _optNames = ti.wma.info.optionalOutputs;
  console.log(`Optional outputs: ${_optNames.join(', ')}`);
  console.log();

  // Enable all optional outputs
  const [_allOut] = ti.wma.indicator([close], options, _optNames.map(() => true));
  ...
  const [_firstOut] = ti.wma.indicator([close], options, _optNames.map((_, i) => i === 0));
