...
function main() {
  ...
  console.log(`Minimum data for accuracy (6 decimals): ${ti.mass.minDataAccuracy(options, 6)}`);
  console.log();

  const [outputs] = ti.mass.indicator([high, low], options);
  ...
  const [outputs2, state2] = ti.mass.indicator([partialHigh, partialLow], options);
  ...
  try {
    ...
    simdOutputs.forEach((output, i) => console.log(`Asset ${i + 1} Mass Index Line: ${output[0]}`));
  ...
  console.log();
  try {
    const [simdOptOutputs] = ti.mass.simdByOptions([expandedHigh, expandedLow], simdOptions);
    simdOptOutputs.forEach((output, i) => console.log(`Option set ${i + 1} Mass Index Line (first 5): ${output[0].slice(0, 5)}`));
