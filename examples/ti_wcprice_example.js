...
function main() {
  ...
  const [outputs] = ti.wcprice.indicator([high, low, close], options);
  console.log(`Full Weighted Close Price Line: ${outputs[0]}`);

  const partialHigh = high.slice(0, -5);
  const partialLow = low.slice(0, -5);
  const partialClose = close.slice(0, -5);
  const [outputs2, state2] = ti.wcprice.indicator(
    [partialHigh, partialLow, partialClose],
    options,
  );
  console.log(`\nPartial Weighted Close Price Line: ${outputs2[0]}`);
  console.log("\nDemonstrating state continuation...");
  console.log(
    "State info: WCPRICE State - internal state for Weighted Close Price",
  );
  ...
  console.log(`Final Weighted Close Price Line: ${finalOutputs[0]}`);
  console.log(
  ...
  try {
    const [simdOutputs] = ti.wcprice.simdByAssets(simdInputs, options);
    simdOutputs.forEach((output, i) =>
      console.log(`Asset ${i + 1} Weighted Close Price values: ${output[0]}`),
    );
    console.log("\nVerification - calculating each asset individually:");
    simdInputs.forEach((inp, i) => {
      const [o] = ti.wcprice.indicator(inp, options);
