...
pub fn trvi_indicator(
    ...
    let (outputs, inner) =
        rust_trvi::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(&env, outputs, TrviState { inner })
}
...
#[napi]
pub fn trvi_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_trvi::min_data_accuracy(&options, decimals as usize) as u32
}
