...
use tulip_rs::indicators::ccfisher::{
    CcFisher, Indicator, IndicatorByOptions, IndicatorState, TIndicatorState, INPUTS, OPTIONS,
};
...
pub fn ccfisher_indicator(
    ...
    let (outputs, inner) =
        CcFisher::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
...
pub fn ccfisher_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OPTIONS] = options
        .try_into()
        .unwrap_or_else(|_| panic!("Expected {OPTIONS} options"));
    CcFisher::min_data(&option_arr) as u32
}
...
pub fn ccfisher_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => CcFisher::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => CcFisher::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => CcFisher::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => CcFisher::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
...
pub fn ccfisher_simd_by_options(
    ...
    let (outs, states_inner) = match n {
        2 => CcFisher::indicator_by_options::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => CcFisher::indicator_by_options::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => CcFisher::indicator_by_options::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => CcFisher::indicator_by_options::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
