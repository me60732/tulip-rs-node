...
use tulip_rs::indicators::ccfisher as rust_ccfisher;

use crate::utils::{info_to_object, js_pair, map_error, inputs_to_array, vecs_to_float64arrays, InfoObject};

const IW: usize = rust_ccfisher::INPUTS_WIDTH;
const OW: usize = rust_ccfisher::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct CcfisherState {
    pub(crate) inner: rust_ccfisher::IndicatorState,
}
...
impl CcfisherState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_ccfisher::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_ccfisher::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the CCFisher indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[alpha]`
/// Mandatory outputs: `fisher`, `signal` | Optional: `[want_trendmode, want_cycle, want_peak]`
#[napi]
pub fn ccfisher_indicator(
    env: Env,
    inputs: Vec<Float64Array>,
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
    ...
    let (outputs, inner) =
        rust_ccfisher::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), CcfisherState { inner })
}

/// Static metadata for CCFisher.
#[napi]
pub fn ccfisher_info() -> InfoObject {
    info_to_object(rust_ccfisher::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn ccfisher_min_data(options: Vec<f64>) -> u32 {
    rust_ccfisher::min_data(&options) as u32
}


// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through CCFisher in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn ccfisher_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ccfisher::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ccfisher::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ccfisher::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ccfisher::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<CcfisherState> = states_inner
        .into_iter()
        .map(|inner| CcfisherState { inner })
        .collect();
...
/// Run N option-sets against the same CCFisher input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[close]]`   `options_list`: `[N][1]`
#[napi]
pub fn ccfisher_simd_by_options(
    env: Env,
    inputs: Vec<Float64Array>,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ccfisher::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ccfisher::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ccfisher::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ccfisher::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
