...
use tulip_rs::indicators::emv::{
    Emv, Indicator, IndicatorState, TIndicatorState, INPUTS, OPTIONS,
};
...
pub fn emv_indicator(
    ...
    let (outputs, inner) =
        Emv::indicator(&input_arr, &option_arr, optional_outputs.as_deref()).map_err(map_error)?;
...
pub fn emv_info() -> InfoObject {
    info_to_object(Emv::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn emv_min_data(_options: Vec<f64>) -> u32 {
    Emv::min_data(&[]) as u32
}
...
pub fn emv_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Emv::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Emv::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Emv::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Emv::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
