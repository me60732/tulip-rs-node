...
pub fn ad_indicator(
    ...
    let (outputs, inner) =
        Ad::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
...
#[napi]
pub fn ad_min_data(_options: Vec<f64>) -> u32 {
    Ad::min_data(&[]) as u32
}
...
pub fn ad_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Ad::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Ad::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Ad::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Ad::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
