...
pub fn linreg_indicator(
    ...
    let (outputs, inner) =
        rust_linreg::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), LinregState { inner })
}
