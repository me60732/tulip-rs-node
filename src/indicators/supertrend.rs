...
pub struct SupertrendState {
    pub(crate) inner: rust_supertrend::IndicatorState,
}
...
impl SupertrendState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_supertrend::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_supertrend::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the SuperTrend indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[high, low, close]]`   `options`: `[period, step]`
/// Mandatory outputs: `supertrend` | Optional: `[want_atr, want_tr, want_medprice]`
#[napi]
pub fn supertrend_indicator(
    env: Env,
    inputs: Vec<Float64Array>,
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
    ...
    let (outputs, inner) =
        rust_supertrend::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
        vecs_to_float64arrays(outputs),
        SupertrendState { inner },
    )
}

/// Static metadata for SuperTrend.
#[napi]
pub fn supertrend_info() -> InfoObject {
...
/// Run N assets through SuperTrend in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][3][data_len]`
#[napi]
pub fn supertrend_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_supertrend::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_supertrend::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_supertrend::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_supertrend::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<SupertrendState> = states_inner
        .into_iter()
        .map(|inner| SupertrendState { inner })
        .collect();
...
/// Run N option-sets against the same SuperTrend input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[high, low, close]]`   `options_list`: `[N][2]`
#[napi]
pub fn supertrend_simd_by_options(
    env: Env,
    inputs: Vec<Float64Array>,
    ...
    let (outs, states_inner) = match n {
        2 => rust_supertrend::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_supertrend::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_supertrend::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_supertrend::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
