...
use tulip_rs::indicators::ao::{Ao, Indicator, IndicatorState, TIndicatorState, INPUTS};
...
pub fn ao_indicator(
    ...
    let (outputs, inner) =
        Ao::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
...
pub fn ao_info() -> InfoObject {
    info_to_object(Ao::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn ao_min_data(_options: Vec<f64>) -> u32 {
    Ao::min_data(&[]) as u32
}
...
pub fn ao_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Ao::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Ao::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Ao::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Ao::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
