...
const IW: usize = rust_sma::INPUTS_WIDTH;
const OW: usize = rust_sma::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct SmaState {
    pub(crate) inner: rust_sma::IndicatorState,
}
...
impl SmaState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_sma::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_sma::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
pub fn sma_indicator(
    ...
    let (outputs, inner) =
        rust_sma::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
...
pub fn sma_min_data(options: Vec<f64>) -> u32 {
    rust_sma::min_data(&options) as u32
}

/// Minimum input bars needed to achieve a given decimal accuracy.
#[napi]
pub fn sma_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_sma::min_data_accuracy(&options, decimals as usize) as u32
}
