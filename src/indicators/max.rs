...
#[napi]
pub fn max_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) = rust_max::indicator(&input_arr, &option_arr, None).map_err(map_error)?;
