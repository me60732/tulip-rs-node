...
const IW: usize = rust_avgprice::INPUTS_WIDTH;
const OW: usize = rust_avgprice::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct AvgpriceState {
    pub(crate) inner: rust_avgprice::IndicatorState,
}
...
impl AvgpriceState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_avgprice::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_avgprice::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the AVGPRICE indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[open], [high], [low], [close]]`
#[napi]
pub fn avgprice_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) =
        rust_avgprice::indicator(&input_arr, &option_arr, optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(&env, outputs, AvgpriceState { inner })
}
...
#[napi]
pub fn avgprice_min_data(options: Vec<f64>) -> u32 {
    rust_avgprice::min_data(&options) as u32
}


/// Minimum input bars needed to achieve a given decimal accuracy.
#[napi]
pub fn avgprice_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_avgprice::min_data_accuracy(&options, decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through AVGPRICE in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][4][data_len]`
#[napi]
pub fn avgprice_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let n = inputs.len();
    ...
    let (outs, states_inner) = match n {
        2 => rust_avgprice::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_avgprice::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_avgprice::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_avgprice::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<AvgpriceState> = states_inner
        .into_iter()
        .map(|inner| AvgpriceState { inner })
        .collect();
