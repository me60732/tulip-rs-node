...
use tulip_rs::indicators::avgprice::{
    AvgPrice, Indicator, IndicatorState, TIndicatorState, INPUTS,
};
...
pub fn avgprice_indicator(
    ...
    let (outputs, inner) =
        AvgPrice::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(
        &env,
...
pub fn avgprice_info() -> InfoObject {
    info_to_object(AvgPrice::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn avgprice_min_data(_options: Vec<f64>) -> u32 {
    AvgPrice::min_data(&[]) as u32
}
...
pub fn avgprice_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => AvgPrice::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => AvgPrice::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => AvgPrice::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => AvgPrice::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
