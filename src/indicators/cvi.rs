...
#[napi]
pub struct CviState {
    pub(crate) inner: rust_cvi::IndicatorState,
}
...
impl CviState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_cvi::IndicatorState>(buf.as_ref())
...
#[napi]
pub fn cvi_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) = rust_cvi::indicator(&input_arr, &option_arr, None).map_err(map_error)?;
...
#[napi]
pub fn cvi_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_cvi::min_data_accuracy(&options, decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through CVI in a single SIMD pass (N = 2 | 4 | 8 | 16).
...
pub fn cvi_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => rust_cvi::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        4 => rust_cvi::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        8 => rust_cvi::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        16 => rust_cvi::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
    ...
    let states: Vec<CviState> = states_inner
        .into_iter()
...
/// Run N option-sets against the same CVI input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
...
pub fn cvi_simd_by_options(
    ...
    let (outs, states_inner) = match n {
        2 => rust_cvi::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
        .map_err(map_error)?,
        4 => rust_cvi::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
        .map_err(map_error)?,
        8 => rust_cvi::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
        .map_err(map_error)?,
        16 => rust_cvi::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
