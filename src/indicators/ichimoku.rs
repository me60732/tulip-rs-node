...
pub struct IchimokuState {
    pub(crate) inner: rust_ichimoku::IndicatorState,
}
...
impl IchimokuState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_ichimoku::IndicatorState>(buf.as_ref())
...
/// Run the Ichimoku indicator. Returns `[outputs, state]` as a JS array.
...
/// Note: output series may have different lengths due to ichimoku offsets.
#[napi]
pub fn ichimoku_indicator(
    ...
    let (outputs, inner) =
        rust_ichimoku::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
...
/// Static metadata for Ichimoku.
#[napi]
pub fn ichimoku_info() -> InfoObject {
...
/// Run N assets through Ichimoku in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][3][data_len]`
#[napi]
pub fn ichimoku_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ichimoku::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ichimoku::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ichimoku::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ichimoku::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<IchimokuState> = states_inner
        .into_iter()
...
/// Run N option-sets against the same Ichimoku input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[high, low, close]]`   `options_list`: `[N][2]`
#[napi]
pub fn ichimoku_simd_by_options(
    env: Env,
    inputs: Vec<Float64Array>,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ichimoku::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ichimoku::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ichimoku::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ichimoku::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
