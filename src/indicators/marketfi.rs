...
use tulip_rs::indicators::marketfi::{
    Indicator, IndicatorState, Marketfi, TIndicatorState, INPUTS,
};
...
pub fn marketfi_indicator(
    ...
    let (outputs, inner) =
        Marketfi::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(
        &env,
...
pub fn marketfi_info() -> InfoObject {
    info_to_object(Marketfi::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn marketfi_min_data(_options: Vec<f64>) -> u32 {
    Marketfi::min_data(&[]) as u32
}
...
pub fn marketfi_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Marketfi::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Marketfi::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Marketfi::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Marketfi::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
