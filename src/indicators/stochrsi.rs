...
use tulip_rs::indicators::stochrsi::{
    Indicator, IndicatorByOptions, IndicatorState, StochRsi, INPUTS, OPTIONS,
};
...
pub fn stochrsi_indicator(
    ...
    let (outputs, inner) =
        StochRsi::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
...
pub fn stochrsi_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OPTIONS] = options
        ...
        .map_err(|_| Error::new(Status::InvalidArg, format!("Expected {OPTIONS} options")))
        .unwrap();
    StochRsi::min_data(&option_arr) as u32
}
...
pub fn stochrsi_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => StochRsi::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => StochRsi::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => StochRsi::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => StochRsi::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
...
pub fn stochrsi_simd_by_options(
    ...
    let (outs, states_inner) = match n {
        2 => StochRsi::indicator_by_options::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => StochRsi::indicator_by_options::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => StochRsi::indicator_by_options::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => StochRsi::indicator_by_options::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
