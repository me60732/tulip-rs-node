...
use tulip_rs::indicators::ef as rust_ef;

use crate::utils::{info_to_object, js_pair, map_error, inputs_to_array, vecs_to_float64arrays, InfoObject};

const IW: usize = rust_ef::INPUTS_WIDTH;
const OW: usize = rust_ef::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct EfState {
    pub(crate) inner: rust_ef::IndicatorState,
}
...
impl EfState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_ef::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_ef::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the EF (Efficiency Ratio) indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[real]]`   `options`: `[period]`
#[napi]
pub fn ef_indicator(
    env: Env,
    inputs: Vec<Float64Array>,
    ...
    let (outputs, inner) = rust_ef::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
        .map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), EfState { inner })
}

/// Static metadata for EF.
#[napi]
pub fn ef_info() -> InfoObject {
    info_to_object(rust_ef::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn ef_min_data(options: Vec<f64>) -> u32 {
    rust_ef::min_data(&options) as u32
}

/// Minimum input bars needed to achieve a given decimal accuracy.
#[napi]
pub fn ef_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_ef::min_data_accuracy(&options, decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through EF in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn ef_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ef::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ef::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ef::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ef::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<EfState> = states_inner
        .into_iter()
        .map(|inner| EfState { inner })
        .collect();
...
/// Run N option-sets against the same EF input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[real]]`   `options_list`: `[N][1]`
#[napi]
pub fn ef_simd_by_options(
    env: Env,
    inputs: Vec<Float64Array>,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ef::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ef::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ef::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ef::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
