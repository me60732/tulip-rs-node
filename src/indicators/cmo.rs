...
const IW: usize = rust_cmo::INPUTS_WIDTH;
const OW: usize = rust_cmo::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct CmoState {
    pub(crate) inner: rust_cmo::IndicatorState,
}
...
impl CmoState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_cmo::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_cmo::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the CMO indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[close]]`   `options`: `[period]`
#[napi]
pub fn cmo_indicator(env: Env, inputs: Vec<Float64Array>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr = inputs_to_array::<IW>(&inputs)?;
    ...
    let (outputs, inner) = rust_cmo::indicator(&input_arr, &option_arr, optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), CmoState { inner })
}

/// Static metadata for CMO.
#[napi]
pub fn cmo_info() -> InfoObject {
    info_to_object(rust_cmo::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn cmo_min_data(options: Vec<f64>) -> u32 {
    rust_cmo::min_data(&options) as u32
}


/// Minimum input bars needed to achieve a given decimal accuracy.
#[napi]
pub fn cmo_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_cmo::min_data_accuracy(&options, decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through CMO in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn cmo_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_cmo::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_cmo::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_cmo::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_cmo::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<CmoState> = states_inner
        .into_iter()
        .map(|inner| CmoState { inner })
        .collect();
...
/// Run N option-sets against the same CMO input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[close]]`   `options_list`: `[N][1]`
#[napi]
pub fn cmo_simd_by_options(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_cmo::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_cmo::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_cmo::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_cmo::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
