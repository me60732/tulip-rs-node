...
#[napi]
pub struct DpoState {
    pub(crate) inner: rust_dpo::IndicatorState,
}
...
impl DpoState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_dpo::IndicatorState>(buf.as_ref())
...
#[napi]
pub fn dpo_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) = rust_dpo::indicator(&input_arr, &option_arr, None).map_err(map_error)?;
    js_pair(&env, outputs, DpoState { inner })
}

/// Static metadata for DPO.
#[napi]
pub fn dpo_info() -> InfoObject {
...
/// Run N assets through DPO in a single SIMD pass (N = 2 | 4 | 8 | 16).
...
pub fn dpo_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => rust_dpo::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        4 => rust_dpo::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        8 => rust_dpo::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        16 => rust_dpo::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
    ...
    let states: Vec<DpoState> = states_inner
        .into_iter()
...
/// Run N option-sets against the same DPO input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
...
pub fn dpo_simd_by_options(
    ...
    let (outs, states_inner) = match n {
        2 => rust_dpo::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
        .map_err(map_error)?,
        4 => rust_dpo::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
        .map_err(map_error)?,
        8 => rust_dpo::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
        .map_err(map_error)?,
        16 => rust_dpo::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            None,
        )
