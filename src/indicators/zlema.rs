...
use tulip_rs::indicators::zlema::{
    Indicator, IndicatorByOptions, IndicatorState, TIndicatorState, Zlema, INPUTS, OPTIONS,
};
...
pub fn zlema_indicator(
    ...
    let (outputs, inner) = Zlema::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
        .map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), ZlemaState { inner })
}

/// Static metadata for ZLEMA.
#[napi]
pub fn zlema_info() -> InfoObject {
    info_to_object(Zlema::INFO)
}
...
pub fn zlema_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OPTIONS] = options
        ...
        .map_err(|_| Error::new(Status::InvalidArg, format!("Expected {OPTIONS} options")))
        .unwrap_or([1.0]);
    Zlema::min_data(&option_arr) as u32
}
...
pub fn zlema_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Zlema::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Zlema::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Zlema::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Zlema::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
...
pub fn zlema_simd_by_options(
    ...
    let (outs, states_inner) = match n {
        2 => Zlema::indicator_by_options::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Zlema::indicator_by_options::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Zlema::indicator_by_options::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Zlema::indicator_by_options::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
