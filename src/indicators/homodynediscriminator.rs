...
use tulip_rs::indicators::homodynediscriminator::{
    HomodyneDiscriminator, Indicator, IndicatorState, TIndicatorState, INPUTS,
};
...
pub fn homodynediscriminator_indicator(
    ...
    let (outputs, inner) =
        HomodyneDiscriminator::indicator(&input_arr, &[], optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
        vecs_to_float64arrays(outputs),
        HomodynediscriminatorState { inner },
    )
}

/// Static metadata for Homodyne Discriminator.
#[napi]
pub fn homodynediscriminator_info() -> InfoObject {
    info_to_object(HomodyneDiscriminator::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn homodynediscriminator_min_data(_options: Vec<f64>) -> u32 {
    HomodyneDiscriminator::min_data(&[]) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through Homodyne Discriminator in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn homodynediscriminator_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => HomodyneDiscriminator::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => HomodyneDiscriminator::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => HomodyneDiscriminator::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => HomodyneDiscriminator::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<HomodynediscriminatorState> = states_inner
        .into_iter()
