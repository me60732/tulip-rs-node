...
pub struct HomodynediscriminatorState {
    pub(crate) inner: rust_homodynediscriminator::IndicatorState,
}
...
impl HomodynediscriminatorState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_homodynediscriminator::IndicatorState>(buf.as_ref())
...
pub fn homodynediscriminator_indicator(
    ...
    let (outputs, inner) =
        rust_homodynediscriminator::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
...
pub fn homodynediscriminator_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => rust_homodynediscriminator::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_homodynediscriminator::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_homodynediscriminator::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_homodynediscriminator::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
