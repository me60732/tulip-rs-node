...
pub struct SmaEnvelopeState {
    pub(crate) inner: rust_smaenvelope::IndicatorState,
}

#[napi]
impl SmaEnvelopeState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_smaenvelope::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_smaenvelope::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the SmaEnvelope indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[real]]`   `options`: `[period, percentage]`
#[napi]
pub fn smaenvelope_indicator(
    env: Env,
    inputs: Vec<Vec<f64>>,
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
    ...
    let (outputs, inner) =
        rust_smaenvelope::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(&env, outputs, SmaEnvelopeState { inner })
}

/// Static metadata for SmaEnvelope.
#[napi]
pub fn smaenvelope_info() -> InfoObject {
...
#[napi]
pub fn smaenvelope_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_smaenvelope::min_data_accuracy(&options, decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through SmaEnvelope in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn smaenvelope_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_smaenvelope::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_smaenvelope::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_smaenvelope::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_smaenvelope::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<SmaEnvelopeState> = states_inner
        .into_iter()
        .map(|inner| SmaEnvelopeState { inner })
        .collect();
...
/// Run N option-sets against the same SmaEnvelope input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[real]]`   `options_list`: `[N][2]`
#[napi]
pub fn smaenvelope_simd_by_options(
    env: Env,
    inputs: Vec<Vec<f64>>,
    ...
    let (outs, states_inner) = match n {
        2 => rust_smaenvelope::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_smaenvelope::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_smaenvelope::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_smaenvelope::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
