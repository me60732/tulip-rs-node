...
pub struct UltoscState {
    pub(crate) inner: rust_ultosc::IndicatorState,
}
...
impl UltoscState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_ultosc::IndicatorState>(buf.as_ref())
...
/// Run the UltOsc (Ultimate Oscillator) indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[high, low, close]]`   `options`: `[short_period, medium_period, long_period]`
#[napi]
pub fn ultosc_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) =
        rust_ultosc::indicator(&input_arr, &option_arr, optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(&env, outputs, UltoscState { inner })
}

/// Static metadata for UltOsc.
#[napi]
pub fn ultosc_info() -> InfoObject {
...
#[napi]
pub fn ultosc_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_ultosc::min_data_accuracy(&options, decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through UltOsc in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][3][data_len]`
#[napi]
pub fn ultosc_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ultosc::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ultosc::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ultosc::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ultosc::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<UltoscState> = states_inner
        .into_iter()
...
/// Run N option-sets against the same UltOsc input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[high, low, close]]`   `options_list`: `[N][3]`
#[napi]
pub fn ultosc_simd_by_options(
    env: Env,
    inputs: Vec<Vec<f64>>,
    ...
    let (outs, states_inner) = match n {
        2 => rust_ultosc::by_options::indicator::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_ultosc::by_options::indicator::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_ultosc::by_options::indicator::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_ultosc::by_options::indicator::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
