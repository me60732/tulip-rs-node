...
use tulip_rs::indicators::obv::{Indicator, IndicatorState, Obv, INPUTS, OPTIONS};
...
pub fn obv_indicator(
    ...
    let (outputs, inner) =
        Obv::indicator(&input_arr, &option_arr, optional_outputs.as_deref()).map_err(map_error)?;
...
pub fn obv_info() -> InfoObject {
    info_to_object(Obv::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn obv_min_data(_options: Vec<f64>) -> u32 {
    Obv::min_data(&[]) as u32
}
...
pub fn obv_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Obv::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Obv::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Obv::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Obv::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
