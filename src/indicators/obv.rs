...
/// `inputs`: `[[close, volume]]`
#[napi]
pub fn obv_indicator(env: Env, inputs: Vec<Vec<f64>>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
...
#[napi]
pub fn obv_min_data() -> u32 {
    rust_obv::min_data(&[]) as u32
}


/// Minimum input bars needed to achieve a given decimal accuracy.
#[napi]
pub fn obv_min_data_accuracy(decimals: u32) -> u32 {
    rust_obv::min_data_accuracy(&[], decimals as usize) as u32
}
...
#[napi]
pub fn obv_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let n = inputs.len();
