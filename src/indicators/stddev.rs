...
use tulip_rs::indicators::stddev::{
    Indicator, IndicatorByOptions, IndicatorState, StdDev, INPUTS, OPTIONS,
};
...
pub fn stddev_indicator(
    ...
    let (outputs, inner) = StdDev::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
        .map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), StddevState { inner })
}

/// Static metadata for StdDev.
#[napi]
pub fn stddev_info() -> InfoObject {
    info_to_object(StdDev::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn stddev_min_data(options: Vec<f64>) -> u32 {
    let option_arr: [f64; OPTIONS] = options
        .try_into()
        .map_err(|_| Error::new(Status::InvalidArg, format!("Expected {OPTIONS} options")))
        .unwrap();
    StdDev::min_data(&option_arr) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through StdDev in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn stddev_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => StdDev::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => StdDev::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => StdDev::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => StdDev::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<StddevState> = states_inner
        .into_iter()
        .map(|inner| StddevState { inner })
        .collect();
...
/// Run N option-sets against the same StdDev input in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs`: `[[close]]`   `options_list`: `[N][1]`
#[napi]
pub fn stddev_simd_by_options(
    env: Env,
    inputs: Vec<Float64Array>,
    ...
    let (outs, states_inner) = match n {
        2 => StdDev::indicator_by_options::<2>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => StdDev::indicator_by_options::<4>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => StdDev::indicator_by_options::<8>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => StdDev::indicator_by_options::<16>(
            &input_arr,
            option_refs.as_slice().try_into().unwrap(),
            optional_outputs.as_deref(),
        )
