...
use tulip_rs::indicators::pvi::{Indicator, IndicatorState, Pvi, INPUTS, OPTIONS};
...
pub fn pvi_indicator(
    ...
    let (outputs, inner) =
        Pvi::indicator(&input_arr, &option_arr, optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(&env, vecs_to_float64arrays(outputs), PviState { inner })
}
...
#[napi]
pub fn pvi_min_data(_options: Vec<f64>) -> u32 {
    Pvi::min_data(&[]) as u32
}
...
pub fn pvi_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Pvi::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Pvi::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Pvi::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Pvi::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
