...
/// `inputs`: `[[close, volume]]`
#[napi]
pub fn pvi_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
...
#[napi]
pub fn pvi_min_data_accuracy(options: Vec<f64>, decimals: u32) -> u32 {
    rust_pvi::min_data_accuracy(&options, decimals as usize) as u32
}
...
#[napi]
pub fn pvi_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let n = inputs.len();
