...
const IW: usize = rust_tr::INPUTS_WIDTH;
const OW: usize = rust_tr::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct TrState {
    pub(crate) inner: rust_tr::IndicatorState,
}
...
impl TrState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_tr::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_tr::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
#[napi]
pub fn tr_indicator(env: Env, inputs: Vec<Vec<f64>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
...
#[napi]
pub fn tr_min_data(options: Vec<f64>) -> u32 {
...
#[napi]
pub fn tr_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>, options: Vec<f64>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let n = inputs.len();
