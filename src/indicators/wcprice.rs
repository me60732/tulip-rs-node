...
use tulip_rs::indicators::wcprice::{Indicator, IndicatorState, TIndicatorState, WcPrice, INPUTS};
...
pub fn wcprice_indicator(
    ...
    let (outputs, inner) =
        WcPrice::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
...
#[napi]
pub fn wcprice_min_data(_options: Vec<f64>) -> u32 {
    WcPrice::min_data(&[]) as u32
}
...
pub fn wcprice_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => WcPrice::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => WcPrice::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => WcPrice::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => WcPrice::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
