...
pub struct WcpriceState {
    pub(crate) inner: rust_wcprice::IndicatorState,
}
...
impl WcpriceState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_wcprice::IndicatorState>(buf.as_ref())
...
#[napi]
pub fn wcprice_indicator(env: Env, inputs: Vec<Vec<f64>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) =
        rust_wcprice::indicator(&input_arr, &option_arr, None).map_err(map_error)?;
...
#[napi]
pub fn wcprice_min_data() -> u32 {
    rust_wcprice::min_data(&[]) as u32
}


/// Minimum input bars needed to achieve a given decimal accuracy.
#[napi]
pub fn wcprice_min_data_accuracy(decimals: u32) -> u32 {
    rust_wcprice::min_data_accuracy(&[], decimals as usize) as u32
}

// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through WCPrice in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][3][data_len]`
#[napi]
pub fn wcprice_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>) -> Result<JsObject> {
    let n = inputs.len();
    ...
    let (outs, states_inner) = match n {
        2 => rust_wcprice::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        4 => rust_wcprice::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        8 => rust_wcprice::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
        .map_err(map_error)?,
        16 => rust_wcprice::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            None,
        )
    ...
    let states: Vec<WcpriceState> = states_inner
        .into_iter()
