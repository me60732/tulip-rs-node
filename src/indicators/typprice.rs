...
#[napi]
pub fn typprice_indicator(env: Env, inputs: Vec<Vec<f64>>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
...
#[napi]
pub fn typprice_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>, optional_outputs: Option<Vec<bool>>) -> Result<JsObject> {
    let n = inputs.len();
