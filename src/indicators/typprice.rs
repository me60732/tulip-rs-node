...
#[napi]
pub fn typprice_indicator(env: Env, inputs: Vec<Vec<f64>>) -> Result<JsObject> {
    let input_arr: [&[f64]; IW] = inputs
        .iter()
        .map(|v| v.as_slice())
        .collect::<Vec<_>>()
    ...
    let (outputs, inner) =
        rust_typprice::indicator(&input_arr, &option_arr, None).map_err(map_error)?;
...
#[napi]
pub fn typprice_simd_by_assets(env: Env, inputs: Vec<Vec<Vec<f64>>>) -> Result<JsObject> {
    let n = inputs.len();
