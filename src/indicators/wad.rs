...
use tulip_rs::indicators::wad::{Indicator, IndicatorState, TIndicatorState, Wad, INPUTS};
...
pub fn wad_indicator(
    ...
    let (outputs, inner) =
        Wad::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
...
#[napi]
pub fn wad_min_data(_options: Vec<f64>) -> u32 {
    Wad::min_data(&[]) as u32
}
...
pub fn wad_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Wad::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Wad::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Wad::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Wad::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
