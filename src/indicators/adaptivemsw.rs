...
use tulip_rs::indicators::adaptivemsw::{
    AdaptiveMSW, Indicator, IndicatorState, TIndicatorState, INPUTS,
};
...
pub fn adaptivemsw_indicator(
    ...
    let (outputs, inner) =
        AdaptiveMSW::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(
        &env,
...
pub fn adaptivemsw_info() -> InfoObject {
    info_to_object(AdaptiveMSW::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn adaptivemsw_min_data(_options: Vec<f64>) -> u32 {
    AdaptiveMSW::min_data(&[]) as u32
}
...
pub fn adaptivemsw_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => AdaptiveMSW::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => AdaptiveMSW::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => AdaptiveMSW::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => AdaptiveMSW::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
