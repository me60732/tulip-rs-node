...
const IW: usize = rust_adaptivemsw::INPUTS_WIDTH;
const OW: usize = rust_adaptivemsw::OPTIONS_WIDTH;

// ── State class ──────────────────────────────────────────────────────────────

#[napi]
pub struct AdaptivemswState {
    pub(crate) inner: rust_adaptivemsw::IndicatorState,
}

#[napi]
impl AdaptivemswState {
    ...
    pub fn from_buffer(buf: Buffer) -> Result<Self> {
        bincode::deserialize::<rust_adaptivemsw::IndicatorState>(buf.as_ref())
    ...
    pub fn from_json(json: String) -> Result<Self> {
        serde_json::from_str::<rust_adaptivemsw::IndicatorState>(&json)
            .map(|inner| Self { inner })
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))
    }
}
...
/// Run the Adaptive MSW indicator. Returns `[outputs, state]` as a JS array.
/// `inputs`: `[[price]]`   `options`: `[]`
/// Mandatory outputs: `sine`, `lead_sine` | Optional: `[want_dc_period]`
#[napi]
pub fn adaptivemsw_indicator(
    env: Env,
    inputs: Vec<Float64Array>,
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
    ...
    let (outputs, inner) =
        rust_adaptivemsw::indicator(&input_arr, &option_arr, optional_outputs.as_deref())
            .map_err(map_error)?;
    js_pair(
        &env,
        vecs_to_float64arrays(outputs),
        AdaptivemswState { inner },
    )
}

/// Static metadata for Adaptive MSW.
#[napi]
pub fn adaptivemsw_info() -> InfoObject {
    info_to_object(rust_adaptivemsw::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn adaptivemsw_min_data(options: Vec<f64>) -> u32 {
    rust_adaptivemsw::min_data(&options) as u32
}


// ── SIMD — by assets ─────────────────────────────────────────────────────────

/// Run N assets through Adaptive MSW in a single SIMD pass (N = 2 | 4 | 8 | 16).
/// Returns `[outputs, states]` — both JS arrays of length N.
/// `inputs` shape: `[N][1][data_len]`
#[napi]
pub fn adaptivemsw_simd_by_assets(
    env: Env,
    ...
    let (outs, states_inner) = match n {
        2 => rust_adaptivemsw::by_assets::indicator::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => rust_adaptivemsw::by_assets::indicator::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => rust_adaptivemsw::by_assets::indicator::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => rust_adaptivemsw::by_assets::indicator::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &option_arr,
            optional_outputs.as_deref(),
        )
    ...
    let states: Vec<AdaptivemswState> = states_inner
        .into_iter()
        .map(|inner| AdaptivemswState { inner })
        .collect();
