...
use tulip_rs::indicators::medprice::{
    Indicator, IndicatorState, Medprice, TIndicatorState, INPUTS,
};
...
pub fn medprice_indicator(
    ...
    let (outputs, inner) =
        Medprice::indicator(&input_arr, &[], optional_outputs.as_deref()).map_err(map_error)?;
    js_pair(
        &env,
...
pub fn medprice_info() -> InfoObject {
    info_to_object(Medprice::INFO)
}

/// Minimum number of input bars needed to produce at least one output bar.
#[napi]
pub fn medprice_min_data(_options: Vec<f64>) -> u32 {
    Medprice::min_data(&[]) as u32
}
...
pub fn medprice_simd_by_assets(
    ...
    let (outs, states_inner) = match n {
        2 => Medprice::indicator_by_assets::<2>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        4 => Medprice::indicator_by_assets::<4>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        8 => Medprice::indicator_by_assets::<8>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
        .map_err(map_error)?,
        16 => Medprice::indicator_by_assets::<16>(
            input_refs.as_slice().try_into().unwrap(),
            &[],
            optional_outputs.as_deref(),
        )
